...
from ..simulation.single import inner_pipe, left_boundary, right_boundary
...
def MOCSimulator(tm, results_obj='results', friction='steady'):
    print("Initializing MOCS simulator")
    """ MOC Main Function
    ...
    for ts in range(1,tn):
        ...
        for _,pipe in tm.pipes():
            ...
            if links1[pn] and links2[pn] and \
                ...
                # ===============================
                if not hasattr(pipe, 'H_profile'):
                    pipe.H_profile = []

                if not hasattr(pipe, 'V_profile'):
                    pipe.V_profile = []

                pipe.H_profile.append(HN[pn].copy())
                pipe.V_profile.append(VN[pn].copy())
